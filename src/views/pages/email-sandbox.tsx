...
export const EmailSandboxList: FC<{ emails: StoredEmail[]; models?: any[]; active?: string }> = ({ emails, models, active }) => (
  <Layout title="Email Sandbox - Admin" models={models} active={active}>
    <div class="flex items-center justify-between mb-4">
      <div>
        <h2 class="text-xl font-semibold">Email Sandbox</h2>
        <p class="text-sm opacity-40">{emails.length} captured · Fake provider</p>
      </div>
      {emails.length > 0 && (
      ...
      <div class="card-body p-4">
        {emails.length === 0 ? (
          <div class="text-center py-8 opacity-40">
            ...
            <p class="text-sm mt-1">Use <code>sendEmail()</code> in an action or job</p>
          </div>
        ) : (
...
export const EmailSandboxDetail: FC<{ email: StoredEmail; models?: any[]; active?: string }> = ({ email, models, active }) => (
  <Layout title={"Email #" + email.id + " - Sandbox"} models={models} active={active}>
    <div class="mb-4">
      <a href="/admin/emails" class="text-sm opacity-40 no-underline hover:opacity-100">← Back to inbox</a>
      <h2 class="text-xl font-semibold mt-1">Email #{email.id}</h2>
    </div>
    <div class="card card-border bg-base-100">
      <div class="card-body p-5">
        <dl class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 text-sm">
          ...
          <dt class="font-medium opacity-50">Subject</dt><dd>{email.subject}</dd>
        ...
        {email.html ? (
          <div class="mt-4 border border-base-300 rounded-box overflow-hidden">
            ...
            <iframe src={"/admin/emails/" + email.id + "/preview"} style="width:100%;height:380px;border:none" title="Email preview" />
          </div>
        ) : null}
